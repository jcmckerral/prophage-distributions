...
#     metadf['isolation_date'] = metadf.isolation_date.apply(dc.convert_date)
    ...
    metadf['isolation_country'] = metadf['isolation_country'].str.replace('England', 'United Kingdom')
...
def read_gtdb(tsv_file="../data/bac120_metadata_r202.tsv.gz", use_small_data=False):
    if use_small_data:
    ...
    gtdb = pd.read_csv(tsv_file, compression='gzip', header=0, delimiter="\t", dtype=gtdb_dtypes(), na_values=['na','none'])
